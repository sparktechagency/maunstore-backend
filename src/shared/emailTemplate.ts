...
const PRIMARY_COLOR = '#e3a45c';
const TEXT_COLOR = '#000';
const SUPPORT_EMAIL = 'yaz.vega@raconligroup.com';
...
const createAccount = (values: ICreateAccount) => {
    ...
    <div style="width: 100%; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1); text-align: center;">
        ...
        <p style="color: ${TEXT_COLOR}; font-size: 16px; line-height: 1.5; margin-bottom: 20px;">Your single use code is:</p>
        <div style="background-color: ${PRIMARY_COLOR}; width: 120px; padding: 10px; text-align: center; border-radius: 8px; color: #fff; font-size: 25px; letter-spacing: 2px; margin: 20px auto;">${values.otp}</div>
...
const contact = (values: IContact) => {
    ...
    <div style="width: 100%; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        ...
        <div style="padding: 15px; background-color: #f4f4f4; border-radius: 8px; margin: 20px 0;">
            <p style="color: ${TEXT_COLOR}; font-size: 16px; font-weight: bold;">Your Message Details:</p>
            <p><strong>Name:</strong> ${values.name}</p>
            <p><strong>Email:</strong> ${values.email}</p>
            <p><strong>Subject:</strong> ${values.subject}</p>
            <br/>
...
const resetPassword = (values: IResetPassword) => {
    ...
    <div style="width: 100%; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        <img src="${LOGO_URL}" alt="Logo" style="display: block; margin: 0 auto 20px; width:150px" />
        <div style="text-align: center;">
            <p style="font-size: 16px; line-height: 1.5; margin-bottom: 20px;">Your single use code is:</p>
            <div style="background-color: ${PRIMARY_COLOR}; width: 120px; padding: 10px; text-align: center; border-radius: 8px; color: #fff; font-size: 25px; letter-spacing: 2px; margin: 20px auto;">${values.otp}</div>
            <p style="font-size: 16px; line-height: 1.5;">This code is valid for 3 minutes.</p>
            <p style="color: #b9b4b4; font-size: 14px; line-height: 1.5; margin-top: 20px;">If you didn't request this code, you can safely ignore this email.</p>
        </div>
    </div>
</body>`,
     };
...
const resetPasswordByUrl = (values: IResetPasswordByEmail) => {
    ...
    <div style="width: 100%; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
      ...
      <div style="text-align: center;">
        ...
        <p style="font-size: 16px; line-height: 1.5;">We received a request to reset your password. Click the button below to reset it:</p>
        <a href="${values.resetUrl}" target="_blank" style="display: inline-block; background-color: ${PRIMARY_COLOR}; color: white; text-decoration: none; padding: 12px 20px; border-radius: 8px; font-size: 18px; margin: 20px auto;">Reset Password</a>
        <p style="font-size: 16px; line-height: 1.5; margin-top: 20px;">If you didn’t request this, you can ignore this email.</p>
        <p style="color: #b9b4b4; font-size: 14px;">This link will expire in 10 minutes.</p>
      </div>
    </div>
</body>`,
...
const contactFormTemplate = (values: IHelpContact) => {
    ...
    <div style="width: 100%; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
        <img src="${LOGO_URL}" alt="Logo" style="display: block; margin: 0 auto 20px; width:150px" />
        <div style="text-align: center;">
            <p style="font-size: 16px; line-height: 1.5;">Hello ${values.name},</p>
            <p style="font-size: 16px; line-height: 1.5;">Thank you for reaching out to us. We have received your message:</p>
            <div style="background-color: #f1f1f1; padding: 15px; border-radius: 8px; border: 1px solid #ddd; margin-bottom: 20px;">
                <p style="font-size: 16px; line-height: 1.5;">"${values.message}"</p>
            </div>
            <p style="font-size: 16px; line-height: 1.5;">We will get back to you as soon as possible. Below are the details you provided:</p>
            <p style="font-size: 16px;">Email: ${values.email}</p>
            <p style="font-size: 16px;">Phone: ${values.phone}</p>
            <p style="color: #b9b4b4; font-size: 14px; line-height: 1.5;">If you need immediate assistance, please contact us at <a href="mailto:${SUPPORT_EMAIL}" style="color: ${PRIMARY_COLOR}; text-decoration: none;">${SUPPORT_EMAIL}</a>.</p>
        </div>
    </div>
</body>`,
     };
